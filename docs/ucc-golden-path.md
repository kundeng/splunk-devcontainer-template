...
import ConnectorBuilder from '@splunk/your-builder';

getUserTheme().then((theme) => {
    layout(<ConnectorBuilder />, { theme });
});
```
...
ucc:link:
  cmds:
    - docker exec splunk-dev ln -sf /workspace/ucc/output/your_app /opt/splunk/etc/apps/your_app
