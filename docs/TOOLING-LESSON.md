...
This config was **generated by `adapt-for-ucc.mjs`** (transformation #1), not by `@splunk/create` directly. The scaffold produces a `component.config`-based webpack; the adapt script replaces it with this `base.config` version for UCC overlay use.

```js
// packages/urc-app/webpack.config.js (produced by adapt-for-ucc.mjs)
